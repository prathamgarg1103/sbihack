# Diya — Add-On Flows (Digital Adoption)
### Extension spec for Claude Code

> **Read `diya-context.md` (or `CLAUDE.md`) first.** This file is a *delta* on top of it. All
...
salary→micro-cover). Those prove "Diya recommends products." This add-on adds the flows that
...
**One-liner for the agent's purpose:** Diya is an *agentic Digital Adoption Platform* — unlike
...
1. **No auto-cancellation of third-party apps.** Deep-linking into Netflix/Hotstar to cancel is
   ...
   reminder → redirect the saved amount to a goal.** Never claim Diya cancelled a third-party
...
- Reuse existing: Diya nudge card, EN/हिंदी toggle, voice, "Why am I seeing this?" XAI card,
  consent gate.
...
- Subscription management is the ~2nd most-wanted mobile-banking feature (≈32% rate it
  "extremely valuable"; ~$200/yr/ user wasted on unused subs) — Diya ships it.

**Key presentation line for Flow D:**
> "The first flows drive product adoption. Flow D drives *platform* adoption — Diya finds the
> capabilities a customer would genuinely benefit from and guides them through first-time use,
> moving users beyond UPI and balance checks. Static tools show everyone the same tooltip; Diya
